...
# DONE: Complete predict_mortality
def predict_mortality(model, device, data_loader):
    model.eval()
    probas = []
    with torch.no_grad():
        for inputs, _ in data_loader:
            # Ensure both data and lengths are moved to the correct device
            data_tensor = inputs[0].to(device)
            lengths = inputs[1].to(device)  # Assuming the second element of inputs is the lengths

            # Correctly passing both the data tensor and lengths to the model
            outputs = model((data_tensor, lengths))

            probabilities = torch.softmax(outputs, dim=1)[:, 1]  # Get the probability of the positive class
            probas.extend(probabilities.cpu().numpy())
    return probas


# Evaluate on the validation set
valid_probabilities = predict_mortality(best_model, device, valid_loader)
valid_predictions = [1 if prob > 0.5 else 0 for prob in valid_probabilities]  # Threshold probabilities to get binary predictions

_, _, valid_results = evaluate(best_model, device, valid_loader, criterion)
